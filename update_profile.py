...
Art comes from avatar_to_ascii.py -- rerun it whenever the profile picture changes.
"""
import calendar
import html
import json
...
USER = "JoshBonhage"
BIRTHDAY = date(2001, 8, 21)  # drives the Uptime line
JOINED_YEAR = 2025  # account creation year, never changes
W = 56  # info column width in characters
ART_SIZE, ART_LEAD, ART_TOP, ART_X = 9, 10.4, 45, 20  # portrait cell; more cells = more face, smaller type
INFO_SIZE, INFO_LEAD = 15, 24
INFO_X, CARD_W, CARD_H = 450, 1000, 545
...
def info_lines(s):
    ...
    return [
        ...
        [],
        rule("Contact"),
        kv("Email", "joshua.b@theamazonwhisperer.com"),
        kv("Site", "joshbonhage.com"),
        [],
        rule("GitHub Stats"),
...
def render(mode, stats):
    ...
    out = [
        ...
        f'font-family="Consolas, Menlo, monospace" font-size="{INFO_SIZE}px">',
        f'<rect x="0.5" y="0.5" width="{CARD_W - 1}" height="{CARD_H - 1}" rx="10" fill="{p["bg"]}" stroke="{p["border"]}"/>',
    ]
    for i, line in enumerate(ART.strip("\n").split("\n")):
        out.append(f'<text x="{ART_X}" y="{ART_TOP + i * ART_LEAD}" font-size="{ART_SIZE}px" fill="{p["art"]}" xml:space="preserve">{html.escape(line)}</text>')
    for i, segs in enumerate(info_lines(stats)):
        if not segs:
            continue
        spans = "".join(f'<tspan fill="{p[c]}">{html.escape(t)}</tspan>' for t, c in segs)
        out.append(f'<text x="{INFO_X}" y="{60 + i * INFO_LEAD}" xml:space="preserve">{spans}</text>')
    out.append("</svg>")
    return "\n".join(out)
