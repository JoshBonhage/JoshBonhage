...
ART_SIZE, ART_LEAD, ART_TOP, ART_X = 7.5, 8.7, 40, 20  # portrait cell: more cells read as more face
INFO_X, CARD_W, CARD_H = 460, 1000, 520
...
def render(mode, stats):
    ...
    for i, segs in enumerate(info_lines(stats)):
        ...
        out.append(f'<text x="{INFO_X}" y="{55 + i * 21}" xml:space="preserve">{spans}</text>')
    out.append("</svg>")
    return "\n".join(out)
