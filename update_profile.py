...
ART_SIZE, ART_LEAD, ART_TOP = 10, 11.5, 40  # portrait type: smaller cells = more face
...
def render(mode, stats):
    ...
    for i, line in enumerate(ART.strip("\n").split("\n")):
        out.append(f'<text x="20" y="{ART_TOP + i * ART_LEAD}" font-size="{ART_SIZE}px" fill="{p["art"]}" xml:space="preserve">{html.escape(line)}</text>')
